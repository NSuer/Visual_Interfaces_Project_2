class FocusTimeline {
	...
	initVis() {
		...
		// create SVG drawing area, define its size
	...
	updateVis() {
		...
		// determine distribution of the current dataset by year (with filter applied upstream)
		let aggrData = []
		let xRange = d3.extent(vis.data, d => new Date(d.time))
		console.log("Current time xRange in focusTimeline: " + xRange)

		let keyYear = xRange[0].getFullYear() + 1 // we're not doing 1999; that only has 1 quake.
		while (keyYear <= xRange[1].getFullYear()) {
			aggrData.push([keyYear, d3.filter(vis.data, (d) => new Date(d.time).getFullYear() == keyYear).length])
			keyYear += 1
		}
		...
		d3.map(vis.aggrData, d => console.log(d))

		console.log(d3.max(vis.aggrData, d => d[0]))

		// Set the scale input domains
		vis.xScaleFocus.domain(d3.extent(vis.aggrData, vis.xValue)); // why is this just saying "t is undefined ?????????"
		vis.yScaleFocus.domain(d3.extent(vis.aggrData, vis.yValue));
